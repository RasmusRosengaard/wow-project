...
- **A genuinely live "troll/decoy" current listing can still become a market's reference price.** Inherent to the 2026-07-25 pricing-model change (sell price = current cheapest listing, see above), not a bug in it. Live-confirmed on item 13051 (Witchfury) post-deploy: its `market_key='b:6659'` group (implausible/junk type-28 values) has no legitimate cheap listing at all, only a ~667,999g decoy sitting live on Draenor — junk-tagged buy-side listings correctly match that group and show a real, honest 99%+ discount against a real, honest (but misleading) reference price. The item's *other* group (`market_key='b:6659|m:28=35'`, the real variant) now correctly shows its own accurate ~7,999g price and discount — the originally reported bug is fixed. There is no remaining sale-classification layer to catch "this current listing looks like a decoy," by design; not fixed, not attempted, flagged for the human to decide if it's worth a future guard (e.g. a plausibility check on the current listing itself, separate from anything sale-inference-related).
