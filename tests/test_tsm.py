...
def test_fetch_csv_parses_rows(monkeypatch):
    ...
    assert result[12345] == {"sale_rate": 0.75, "sold_per_day": 3.2, "avg_sale_price": 10200.0}
    assert result[67890] == {"sale_rate": 0.1, "sold_per_day": 0.05, "avg_sale_price": 510.0}
...
def test_refresh_if_stale_fetches_when_cache_is_empty(cache_path, monkeypatch):
    ...
    assert cache.get(12345) == {"sale_rate": 0.75, "sold_per_day": 3.2, "avg_sale_price": 10200.0}
...
def test_refresh_if_stale_keeps_old_data_on_failed_fetch(cache_path, monkeypatch):
    ...
    did_fetch = cache.refresh_if_stale()
    assert did_fetch is False
    assert cache.get(12345) == {"sale_rate": 0.75, "sold_per_day": 3.2, "avg_sale_price": 10200.0}  # still there
...
def test_refresh_if_stale_saves_to_disk(cache_path, monkeypatch):
    ...
    assert saved["items"]["12345"] == {"sale_rate": 0.75, "sold_per_day": 3.2, "avg_sale_price": 10200.0}
...
def test_new_instance_reads_previously_saved_cache(cache_path, monkeypatch):
    ...
    assert fresh_instance.get(12345) == {"sale_rate": 0.75, "sold_per_day": 3.2, "avg_sale_price": 10200.0}
